...
from loss.focal_loss import FocalLossWithSmoothing


def focal_loss(logits, labels, alpha, gamma, is_softmax):
    ...
    if is_softmax:
        ...
        bce_loss = F.binary_cross_entropy(input=logits, target=labels, reduction="none")
...
class ClassBalancedLoss(torch.nn.Module):
    def __init__(self, samples_per_class=None,num_classes = 7, is_smoothing=False, is_softmax=True, beta=0.9999, gamma=2, loss_type="focal"):
        ...
        # self.floss = FocalLossWithSmoothing(class_num=num_classes)
    ...
    def forward(self, x, y):
        ...
        if self.loss_type == "focal":
            cb_loss = focal_loss(x, labels_one_hot, weights, self.gamma, self.is_softmax)
            # cb_loss = self.loss(x)
