...
if os.getcwd().split('/')[-1] == 'main':
    log_file = "logger/train_log.log"
else:
    log_file = "main/logger/train_log.log"
...
## videoMAE size (16, 128, 128)(14, 112, 112)-----(20, 128, 128) (16, 112, 112) 
...
def main():
    ...
    #     if 'model2' in para_name:
    #         para.requires_grad = False
    #     if 'mlp' in para_name:
    #         para.requires_grad = True
    _logger.info(f'Model unfrozen param count:{sum([m.numel() for m in model.parameters() if m.requires_grad])}')
    ...
    weight = (sample.sum() / sample).float().cuda()
...
def train_one_epoch(
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        #     input_bal, target_bal = input[index], target[index]
        #     input = lam * input + (1 - lam) * input_bal
        #     target = [target, target_bal]
            # target = lam * F.one_hot(target) + (1 - lam) * F.one_hot(target_bal)
